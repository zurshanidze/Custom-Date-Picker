...
function toggleDatePicker(e) {
  if (!checkEventPathForClass(e.composedPath(), "dates")) {
    ...
    days_element.classList.toggle("active");
  }
}
...
function checkEventPathForClass(composedPath, selector) {
  for (let i = 0; i < composedPath.length; i++) {
    if (
      composedPath[i].classList &&
      composedPath[i].classList.contains(selector)
    ) {
      return true;
    }
  }
